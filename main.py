...
class Kyb:
    def __init__(self):
        ...
        with open("liste_francais.json", "r") as file:
            self.json_word_liste = json.load(file)
...
class GraphicApp(QWidget):
    def __init__(self):
        ...
        self.json_word_liste_save = None

        self.center()
        self.init_ui()

    def update_list_of_word(self):
        if self.json_word_liste_save != kyb.json_word_liste["start_letter"]:
            self.json_word_liste_save = kyb.json_word_liste["start_letter"]
    ...
    def init_ui(self):
        self.setWindowTitle("Liste d'autocomplétion")
...
if os.path.exists("liste_francais.json"):
    kyb = Kyb()
